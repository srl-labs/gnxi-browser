<script>
	...
	const needField = (dict, kind) => {
		let checkList = [];
		dict.map(function (el) {
			if (el[kind] != '') checkList.push(true);
		});
				...
				{#if ov.length > 1}
					<div class="dropdown-menu" id="dropdown-menu" role="menu">
						<div class="dropdown-content">
							{#each ov as item}
								{#if item !== v}
									<a data-sveltekit-reload class="navbar-item" href="{s}?version={item}">{item}</a>
								{/if}
							{/each}
						</div>
					</div>
			...
			{#if Object.keys(interfaces[p].services).length > 1}
				<div class="navbar-item dropdown is-hoverable">
					<div class="dropdown-trigger">
						<!-- svelte-ignore a11y-missing-attribute -->
						<a class="has-text-white" aria-haspopup="true" aria-controls="dropdown-menu">
							{interfaces[p].name} <i class="bx bx-chevron-down" />
						</a>
					</div>
					<div class="dropdown-menu" id="dropdown-menu" role="menu">
						<div class="dropdown-content">
