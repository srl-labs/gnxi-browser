	...
	<div class="flex-grow-1 flex-shrink-0 m-auto px-10">
		<div class="md:grid md:grid-cols-2 md:gap-20 md:items-center">
			...
			<div class="bg-white dark:bg-gray-800 rounded shadow-xl">
				<div class="bg-gray-50 dark:bg-gray-600 p-4 rounded-t">
					<ul
							...
							<li class="mr-2">
								...
								<a
									...
									class="inline-block px-3 py-2 rounded-lg cursor-pointer {currentPanel === ifcName
										...
										: 'hover:bg-gray-200 hover:text-gray-800 dark:hover:bg-blue-600 dark:hover:text-white'}">{ifcName}</a>
							</li>
						{/each}
					</ul>
				</div>
				<div
					class="h-[300px] max-w-[450px] lg:w-[450px] p-2 overflow-y-auto  scroll-light dark:scroll-dark my-1">
					<ul>
						{#each panelList as item, i}
							<li class="items-center p-4 text-gray-900 hover:bg-gray-100 {i > 0 ? 'border-t dark:border-gray-700' : ''} dark:hover:bg-gray-700">
								<a href={item.href}>
									<p class="dark:text-white">{item.name}</p>
									{#if item.desc}
										<p class="mt-1 font-light text-sm dark:text-gray-400">{item.desc}</p>
									{/if}
								</a>
							</li>
						{/each}
